...
Flink 애플리케이션은 **Domain-Driven Design (DDD)** 원칙을 따라 설계되었으며, **경량화**를 위해 Spring Boot 없이 Kotlin/JVM 기반으로 구현되었습니다:
...
- **테스트 용이성:** 순수 도메인 객체로 단위 테스트 작성 가능
...
`docs/` 아래 문서는 운영 문서, 설계 결정, 학습 노트, 로드맵, 포트폴리오 메모로 분류되어 있습니다. 읽는 순서는 [docs/README.md](docs/README.md)를 기준으로 확인합니다.
...
-   **Watermark:** 5초 (최대 5초 늦게 도착하는 이벤트 처리)
